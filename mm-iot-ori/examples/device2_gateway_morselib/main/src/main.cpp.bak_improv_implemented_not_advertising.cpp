...
// Improv BLE UUIDs
#define GATTS_SERVICE_UUID_IMPROV   0xFF00
#define GATTS_CHAR_UUID_CAPABILITIES 0xFF01
#define GATTS_CHAR_UUID_STATE        0xFF02
#define GATTS_CHAR_UUID_ERROR        0xFF03
#define GATTS_CHAR_UUID_RPC_COMMAND  0xFF04
#define GATTS_CHAR_UUID_RPC_RESULT   0xFF05
#define GATTS_CHAR_UUID_DEVICE_INFO  0xFF06
...
static uint8_t improv_adv_data[] = {
    0x02, 0x01, 0x06,                           // Flags: LE General Discoverable Mode, BR/EDR not supported
    0x0E, 0x09, 'I', 'm', 'p', 'r', 'o', 'v', ' ', 'G', 'a', 't', 'e', 'w', 'a', 'y' // Complete Local Name (14 bytes)
};
...
static void send_improv_state(ImprovState state) {
    ...
    uint8_t pkt[11] = {'I', 'M', 'P', 'R', 'O', 'V', 1, 0x03, 1, (uint8_t)state, 0};
    uint8_t checksum = 0;
    for (int i = 0; i < 10; i++) checksum += pkt[i];
    pkt[10] = checksum;
    uart_write_bytes(IMPROV_UART_PORT, (const char*)pkt, 11);
...
static void send_improv_serial_error(ImprovError error) {
    s_improv_error = error;
    uint8_t pkt[11] = {'I', 'M', 'P', 'R', 'O', 'V', 1, 0x02, 1, (uint8_t)error, 0};
    uint8_t checksum = 0;
    for (int i = 0; i < 10; i++) checksum += pkt[i];
    pkt[10] = checksum;
    uart_write_bytes(IMPROV_UART_PORT, (const char*)pkt, 11);
...
/**
 * Gateway main loop - runs in its own FreeRTOS task to avoid stack overflow
 * Receives UDP packets from HaLow network and processes device data
 */
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
    switch (event) {
        case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
            LOG_INFO("BLE advertisement data configured, starting advertising...");
            esp_ble_gap_start_advertising(&improv_adv_params);
            break;
        case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
            if (param->adv_start_cmpl.status != ESP_BT_STATUS_SUCCESS) {
                LOG_ERROR("BLE advertising start failed with status %d", param->adv_start_cmpl.status);
            } else {
                LOG_INFO("✓ BLE advertising started successfully - device should be visible now");
            }
            break;
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    // Static UUIDs for characteristics and descriptors
    static esp_bt_uuid_t uuid_capabilities = { .len = ESP_UUID_LEN_16, .uuid = { .uuid16 = GATTS_CHAR_UUID_CAPABILITIES } };
    static esp_bt_uuid_t uuid_state = { .len = ESP_UUID_LEN_16, .uuid = { .uuid16 = GATTS_CHAR_UUID_STATE } };
    static esp_bt_uuid_t uuid_error = { .len = ESP_UUID_LEN_16, .uuid = { .uuid16 = GATTS_CHAR_UUID_ERROR } };
    static esp_bt_uuid_t uuid_rpc_command = { .len = ESP_UUID_LEN_16, .uuid = { .uuid16 = GATTS_CHAR_UUID_RPC_COMMAND } };
    static esp_bt_uuid_t uuid_rpc_result = { .len = ESP_UUID_LEN_16, .uuid = { .uuid16 = GATTS_CHAR_UUID_RPC_RESULT } };
    static esp_bt_uuid_t uuid_device_info = { .len = ESP_UUID_LEN_16, .uuid = { .uuid16 = GATTS_CHAR_UUID_DEVICE_INFO } };
    static esp_bt_uuid_t uuid_cccd = { .len = ESP_UUID_LEN_16, .uuid = { .uuid16 = 0x2902 } }; // Client Characteristic Configuration Descriptor
    static esp_gatt_srvc_id_t service_id = { .id = { .uuid = { .len = ESP_UUID_LEN_16, .uuid = { .uuid16 = GATTS_SERVICE_UUID_IMPROV } }, .inst_id = 0 }, .is_primary = true };
    
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            LOG_INFO("GATTS registered, setting up BLE device...");
            improv_gatts_if = gatts_if;
            esp_ble_gap_set_device_name("Improv Gateway");
            LOG_INFO("Device name set to: Improv Gateway");
            esp_ble_gap_config_adv_data_raw((uint8_t*)improv_adv_data, sizeof(improv_adv_data));
            LOG_INFO("Advertisement data configured (%d bytes)", sizeof(improv_adv_data));
            esp_ble_gatts_create_service(gatts_if, &service_id, IDX_CHAR_VAL_MAX);
        } else {
            LOG_ERROR("GATTS registration failed, error code = %x", param->reg.status);
        }
    } else if (event == ESP_GATTS_CREATE_EVT) {
        if (param->create.status == ESP_GATT_OK) {
            LOG_INFO("Improv Service created successfully");
        ...
        if (param->add_char.status == ESP_GATT_OK) {
            if (param->add_char.char_uuid.uuid.uuid16 == GATTS_CHAR_UUID_CAPABILITIES) improv_handle_table[IDX_CHAR_CAPABILITIES_VAL] = param->add_char.attr_handle;
            else if (param->add_char.char_uuid.uuid.uuid16 == GATTS_CHAR_UUID_STATE) improv_handle_table[IDX_CHAR_STATE_VAL] = param->add_char.attr_handle;
            else if (param->add_char.char_uuid.uuid.uuid16 == GATTS_CHAR_UUID_ERROR) improv_handle_table[IDX_CHAR_ERROR_VAL] = param->add_char.attr_handle;
            else if (param->add_char.char_uuid.uuid.uuid16 == GATTS_CHAR_UUID_RPC_COMMAND) improv_handle_table[IDX_CHAR_RPC_COMMAND_VAL] = param->add_char.attr_handle;
            else if (param->add_char.char_uuid.uuid.uuid16 == GATTS_CHAR_UUID_RPC_RESULT) improv_handle_table[IDX_CHAR_RPC_RESULT_VAL] = param->add_char.attr_handle;
            else if (param->add_char.char_uuid.uuid.uuid16 == GATTS_CHAR_UUID_DEVICE_INFO) improv_handle_table[IDX_CHAR_DEVICE_INFO_VAL] = param->add_char.attr_handle;
        } else {
            LOG_ERROR("Add char failed, error code = %x", param->add_char.status);
        }
    } else if (event == ESP_GATTS_ADD_CHAR_DESCR_EVT) {
        if (param->add_char_descr.status == ESP_GATT_OK) {
            if (param->add_char_descr.descr_uuid.uuid.uuid16 == 0x2902) { // CCCD UUID
                // Store the descriptor handle based on context (simplified - just store sequentially)
            ...
            LOG_ERROR("Add char descriptor failed, error code = %x", param->add_char_descr.status);
        }
        ...
        if (param->read.handle == improv_handle_table[IDX_CHAR_CAPABILITIES_VAL]) {
            uint8_t capabilities = 0x03; // 0x01 for Serial, 0x02 for BLE. Both supported.
            rsp.attr_value.len = 1;
            rsp.attr_value.value[0] = capabilities;
        ...
        if (param->write.handle == improv_handle_table[IDX_CHAR_RPC_COMMAND_VAL]) {
            ...
            if (command == WIFI_SETTINGS && data_len > 2) {
                ...
                LOG_INFO("Improv BLE received SSID: %s", new_ssid);
                save_wifi_credentials(new_ssid, new_pass);

                send_improv_state(STATE_PROVISIONED);
                send_ble_improv_rpc_result_notification((uint8_t[]){0x01, 0x00}, 2); // Command 0x01, Error 0x00 (None)
                ...
                uint8_t rpc_result[info_len + 2];
                rpc_result[0] = GET_DEVICE_INFO; // Command ID
                rpc_result[1] = ERROR_NONE; // Error code
                memcpy(&rpc_result[2], info, info_len);
                send_ble_improv_rpc_result_notification(rpc_result, info_len + 2);
            } else {
                send_improv_error(ERROR_UNKNOWN_RPC);
                send_ble_improv_rpc_result_notification((uint8_t[]){command, ERROR_UNKNOWN_RPC}, 2);
            }
            ...
            if (param->write.len == 2 && param->write.value[0] == 0x01 && param->write.value[1] == 0x00) {
                LOG_INFO("Client enabled notifications for handle %d", param->write.handle);
            } else if (param->write.len == 2 && param->write.value[0] == 0x00 && param->write.value[1] == 0x00) {
                LOG_INFO("Client disabled notifications for handle %d", param->write.handle);
            }
        }
        esp_ble_gatts_send_response(gatts_if, param->write.conn_id, param->write.trans_id, ESP_GATT_OK, NULL);
    } else if (event == ESP_GATTS_CONNECT_EVT) {
        improv_conn_id = param->connect.conn_id;
        LOG_INFO("BLE client connected: %d", improv_conn_id);
        // Update state to AWAITING_AUTHORIZATION if not already provisioned
        if (s_improv_state != STATE_PROVISIONED) {
            send_improv_state(STATE_AWAITING_AUTHORIZATION);
        }
    } else if (event == ESP_GATTS_DISCONNECT_EVT) {
        LOG_INFO("BLE client disconnected: %d", param->disconnect.conn_id);
        improv_conn_id = 0xFFFF;
        esp_ble_gap_start_advertising(&improv_adv_params); // Restart advertising
    } else if (event == ESP_GATTS_MTU_EVT) {
        LOG_INFO("MTU changed to %d", param->mtu.mtu);
    }
}
...
static esp_err_t initialize_ble_improv(void) {
    ...
    LOG_INFO("BLE Improv initialized");
    return ESP_OK;
}
...
static void improv_task(void *pvParameters) {
    ...
    uint8_t data[128];
    LOG_INFO("Improv serial task started on UART0");
    ...
    while (1) {
        ...
        if (len >= 9 && memcmp(data, IMPROV_PREFIX, 6) == 0) { // Minimum packet size for header + 1 byte payload + checksum
            ...
            if (len < (9 + payload_len + 1)) { // 6 prefix + 1 version + 1 type + 1 payload_len + payload_len + 1 checksum
                LOG_WARN("Improv Serial: Received packet too short for declared payload length. len=%d, payload_len=%d", len, payload_len);
            ...
            if (calculated_checksum != received_checksum) {
                LOG_WARN("Improv Serial: Checksum mismatch. Expected %02x, Got %02x. Packet Type: 0x%02x", calculated_checksum, received_checksum, packet_type);
                // For checksum errors, it's often better to just drop the packet to avoid spamming.
                continue;
            }
            ...
            if (packet_type == 0x01) { // This is a Command packet
                if (payload_len < 1) { // A command packet must have at least 1 byte for the command ID
                    LOG_WARN("Improv Serial: Command packet with empty payload.");
                    send_improv_serial_error(ERROR_INVALID_RPC);
                    continue;
                }
                ...
                if (command_id == WIFI_SETTINGS) {
                    ...
                    if (payload_len < 3) { // Minimum: 1 (ssid_len=1) + 1 (ssid) + 1 (pass_len=0) + 0 (pass) = 3
                        LOG_WARN("Improv Serial: Malformed WIFI_SETTINGS command (payload too short). payload_len=%d", payload_len);
                        send_improv_serial_error(ERROR_INVALID_RPC);
                        continue;
                    }
                    ...
                    if (ssid_len > 32 || (10 + ssid_len) > (9 + payload_len)) { // Check bounds for ssid_len
                        LOG_WARN("Improv Serial: Invalid SSID length or packet too short for SSID. ssid_len=%d, payload_len=%d", ssid_len, payload_len);
                        send_improv_serial_error(ERROR_INVALID_RPC);
                        continue;
                    }
                    char new_ssid[33] = {0};
                    ...
                    if (pass_len_idx >= (9 + payload_len)) { // Check bounds for pass_len_idx
                        LOG_WARN("Improv Serial: Packet too short for password length. pass_len_idx=%d, payload_len=%d", pass_len_idx, payload_len);
                        send_improv_serial_error(ERROR_INVALID_RPC);
                        continue;
                    }
                    uint8_t pass_len = data[pass_len_idx];
                    if (pass_len > 64 || (pass_len_idx + 1 + pass_len) > (9 + payload_len)) { // Check bounds for pass_len
                        LOG_WARN("Improv Serial: Invalid password length or packet too short for password. pass_len=%d, payload_len=%d", pass_len, payload_len);
                        send_improv_serial_error(ERROR_INVALID_RPC);
                        continue;
                    }
                    char new_pass[65] = {0};
                    ...
                    LOG_INFO("Improv received SSID: %s", new_ssid);
                    save_wifi_credentials(new_ssid, new_pass);
                    ...
                    if (payload_len != 1) { // GET_DEVICE_INFO command has no additional payload
                        LOG_WARN("Improv Serial: Malformed GET_DEVICE_INFO command (unexpected payload). payload_len=%d", payload_len);
                        send_improv_serial_error(ERROR_INVALID_RPC);
                        continue;
                    }
                const char* info = "MorseMicro-ESP32-Gateway";
                ...
                if (info_len + 10 > sizeof(resp)) { // 6+1+1+1+info_len+1
                    LOG_ERROR("Improv Serial: Device info too long for response buffer.");
                    // Cannot send response, just log.
                    continue;
                }
                ...
                uart_write_bytes(IMPROV_UART_PORT, (const char*)resp, 9 + info_len + 1);
                    ...
                    if (payload_len != 1) { // IDENTIFY command has no additional payload
                        LOG_WARN("Improv Serial: Malformed IDENTIFY command (unexpected payload). payload_len=%d", payload_len);
                        send_improv_serial_error(ERROR_INVALID_RPC);
                        continue;
                    }
                    ...
                    if (payload_len != 1) { // GET_CURRENT_STATE command has no additional payload
                        LOG_WARN("Improv Serial: Malformed GET_CURRENT_STATE command (unexpected payload). payload_len=%d", payload_len);
                        send_improv_serial_error(ERROR_INVALID_RPC);
                        continue;
                    }
                    ...
                    LOG_WARN("Improv Serial: Unknown command ID 0x%02x", command_id);
                    send_improv_serial_error(ERROR_UNKNOWN_RPC);
                }
            } else { // Not a Command packet type (e.g., Error, State, RPC Result received from host - which shouldn't happen)
                LOG_WARN("Improv Serial: Received unexpected packet type 0x%02x from host.", packet_type);
...
extern "C" void app_main(void) {
    ...
    if (load_wifi_credentials()) {
        LOG_INFO("Provisioning found in NVS");
    ...
    if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
        ...
        if (err != ESP_OK) {
            LOG_WARN("Failed to connect to backend server");
        }
    } else {
        LOG_WARN("Backend WiFi connection failed");
    }
